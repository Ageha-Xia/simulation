...
def symmetry_sample(n):
    ...
    while i < n:
        ...
        if eta <= (x / 2) / (1 / 2): # f(x) = 1/2, h(x) = x/2
            xs[i] = x
        else:
            xs[i] = -x
        i = i + 1
...
parser.add_argument('--method', type=str, default='direct', nargs='?', help='Method to generate samples, direct or symmetry') 
